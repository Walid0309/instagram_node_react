...
function Signin({ history }) {
	...
	const { showPassword, changeShowPassword } = useContext(GlobalContext);
		...
		axios
			...
			.then((res) => {
				...
				localStorage.setItem(
					'user',
					JSON.stringify({ email: res.data.email, name: res.data.name, id: res.data._id })
				);
				history.push('/');
			})
			.catch((err) => {
	...
	return (
		<div>
			...
			<form>
				...
				/>
				<button onClick={handlePassword}>show password</button>
				<br />
